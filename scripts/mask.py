...
parser.add_argument(
    ...
    "--outputFile",
    help=(
        "Path to output file. File extensions '.txt' and '.txt.gz' will export to a "
        "text file, with compression in the latter case. This is slowest and large. "
        "Best for human readable output. '.npy' will export to an uncompressed numpy "
        ".npy file. This is the fastest option but large. '.npz' (default) will "
        "export to a compressed numpy .npz file. This is slower than .npy but much "
...
def coord2index(x, y, referenceCoordinates=(args.xMin, args.yMin)):
    ...
    Intended to handle input coordinates that don't align with matrix coordinates due
    to origin position.
